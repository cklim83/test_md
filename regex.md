...
Note: not everything is a genuine control character escape. For example:
...
Example of using word boundary `\b`
```Ruby
pattern = /\b\w\w\w\b/
...
Example of using non-word boundary `\B`
```Ruby
pattern = /\Bjohn/i
...
Like in the previous example, to match at least 3 digits, we could use `/\b\d\d\d+\b/` instead of `/\b\d\d\d\d*\b/`

**Zero or One**
`?` quantifier to match 0 or 1 occurrence of a pattern to its left
```plaintext
/coo?t/ matches all the string below
...
```Plaintext
`/\b\d\d\d\d-?\d\d-?\d\d\b/` matches all below
